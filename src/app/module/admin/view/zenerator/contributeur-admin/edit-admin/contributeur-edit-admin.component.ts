...
@Component({
    selector: 'app-contributeur-edit-admin',
    templateUrl: './contributeur-edit-admin.component.html',
    styleUrls: ['./contributeur-edit-admin.component.css']
})
export class ContributeurEditAdminComponent implements OnInit {
...
// methods
    ...
    deleteBuckets(p: BucketVo) {
        this.selectedContributeur.bucketsVo.forEach((element, index) => {
            if (element === p) {
                this.selectedContributeur.bucketsVo.splice(index, 1);
            }
        });
    }
