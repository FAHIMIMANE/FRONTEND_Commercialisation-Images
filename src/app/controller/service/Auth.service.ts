...
export class AuthService {
    ...
    constructor(private http: HttpClient, private tokenService: TokenService, private router: Router) { }
       ...
       public loginContributeur(username: string, password: string) {
       this.http.post<any>(this.API + 'login', { username, password }, { observe: 'response' }).subscribe(
            resp => {
                this.error = null;
                const jwt = resp.headers.get('Authorization');
                ...
                this.loadInfos();
                console.log('you are logged in successfully');
                this.router.navigate(['/' + environment.rootAppUrl + '/contributeur']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
    ...
    public registerClient() {
        console.log(this.user);
        this.http.post<any>(this.API + 'api/client/client/', this.user, {observe: 'response'}).subscribe(
            resp => {
    ...
    public registerContributeur() {
        console.log(this.user);
        this.http.post<any>(this.API + 'api/contributeur/contributeur/', this.user, {observe: 'response'}).subscribe(
            resp => {
